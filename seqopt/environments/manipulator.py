...
_P_IN_HAND = 0.0  # Probabillity of object-in-hand initial state
_P_IN_TARGET = 0  # Probabillity of object-in-target initial state
_P_ON_GROUND = 1.0  # Probability of object-on-ground initial state
...
def make_model(use_peg, use_block, insert):
  """Returns a tuple containing the model XML string and a dict of assets."""
  xml_string = resources.GetResource(os.path.join(os.path.dirname(__file__), 'manipulator.xml'))
  parser = etree.XMLParser(remove_blank_text=True)
  mjcf = etree.XML(xml_string, parser)

  assert not(use_peg and use_block), "Cannot set both use_peg and use_block to True!"
  ...
  if use_peg:
    ...
    required_props = ['block', 'target_block']
    if insert:
      required_props += ['cup']
  else:
...
def bring_ball(fully_observable=True, time_limit=_TIME_LIMIT, random=None,
  ...
  use_block = False
  insert = False
  physics = Physics.from_xml_string(*make_model(use_peg,  use_block, insert))
  task = Bring(use_peg=use_peg, use_block=use_block, insert=insert,
...
@SUITE.add('hard')
def bring_block(fully_observable=True, time_limit=_TIME_LIMIT, random=None,
                environment_kwargs=None):
  """Returns manipulator bring task with the block prop."""
  use_peg = False
  use_block = True
  insert = False
  physics = Physics.from_xml_string(*make_model(use_peg, use_block, insert))
...
@SUITE.add('hard')
def insert_block(fully_observable=True, time_limit=_TIME_LIMIT, random=None,
                 environment_kwargs=None):
  """Returns manipulator insert task with the ball prop."""
  use_peg = False
  use_block = True
...
class Bring(base.Task):
  ...
  def __init__(self, use_peg, use_block, insert, fully_observable, random=None):
  ...
  def initialize_episode(self, physics):
    ...
    while penetrating:
      ...
      object_init_probs = [_P_IN_HAND, _P_IN_TARGET, _P_ON_GROUND, 1-_P_IN_HAND-_P_IN_TARGET-_P_ON_GROUND]

      # To account for rounding errors
      for i in range(len(object_init_probs)):
        object_init_probs[i] = np.clip(object_init_probs[i], 0.0, 1.0)

      init_type = choice(['in_hand', 'in_target', 'on_ground', 'uniform'],
                         p=object_init_probs)
      ...
      elif init_type == 'on_ground':
        physics.after_reset()
        object_x = target_x
        object_z = 0.025
        object_angle = 0.0
        data.qvel[self._object + '_x'] = 0.0
        data.qvel[self._object + '_z'] = 0.0
      else:
  ...
  def _block_reward(self, physics):
    """Returns a reward for bringing the ball prop to the target."""
    # TODO(someshdaga): Add orientation error for block
    # return self._is_close(physics.site_distance('block', 'target_block'))

    # Constants
    grasp_to_obj_close = 0.01
    obj_to_target_close = 0.01
    min_grasp_force = 0.25

    grasp_to_obj_dist = physics.site_distance('grasp', 'block')
    obj_to_target_dist = physics.site_distance('block', 'target_block')
    grasp_force = np.mean(physics.touch())

    grasp_success = (grasp_to_obj_dist <= grasp_to_obj_close and grasp_force >= min_grasp_force)
    place_success = (obj_to_target_dist <= obj_to_target_close)

    reach_reward = 1.0 - (np.tanh(np.arctanh(np.sqrt(0.95)) * grasp_to_obj_dist / 0.3)) ** 2
    grasp_reward = 1.0 if grasp_success else 0.0
    place_reward = 1.0 - (np.tanh(np.arctanh(np.sqrt(0.95)) * obj_to_target_dist / 0.3)) ** 2
    place_reward = grasp_reward * (1.0 + place_reward)

    return reach_reward + grasp_reward + place_reward

  def get_reward(self, physics):
    """Returns a reward to the agent."""
    if self._use_peg:
      return self._peg_reward(physics)
    elif self._use_block:
      return self._block_reward(physics)
    else:
