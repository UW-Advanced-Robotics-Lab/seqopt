...
if __name__ == '__main__':
    ...
    env_id = 'FetchModifiedPickAndPlace-v1'
    ...
    if args.continue_training != '':
        ...
        if args.count_based_exploration:
            def features_extractor(states: th.Tensor):
                ...
                features = states[..., np.concatenate([manipulator_utils.FETCH_INDEX_DICT['reach_dist'],
                                                       manipulator_utils.FETCH_INDEX_DICT['fingertip_dist']])]
                return features
        ...
        reach_actor_params.observation_mask = np.concatenate([
            manipulator_utils.FETCH_INDEX_DICT['object_pos'],
            manipulator_utils.FETCH_INDEX_DICT['grip_pos'],
            manipulator_utils.FETCH_INDEX_DICT['rel_object_pos'],
            manipulator_utils.FETCH_INDEX_DICT['gripper_state'],
            manipulator_utils.FETCH_INDEX_DICT['object_rot'],
        ])
        reach_actor_params.action_mask = np.arange(3)

        # Set critic parameters
        reach_critic_params.net_arch = [300, 200]
        reach_critic_params.observation_mask = np.concatenate([
            manipulator_utils.FETCH_INDEX_DICT['object_pos'],
            manipulator_utils.FETCH_INDEX_DICT['target_pos'],
            manipulator_utils.FETCH_INDEX_DICT['grip_pos'],
            manipulator_utils.FETCH_INDEX_DICT['rel_object_pos'],
            manipulator_utils.FETCH_INDEX_DICT['gripper_state']
        ])
        ...
        # Option 1: GRASPING
        # ------------------
        grasp_actor_params = ActorParams(default_action=th.Tensor([0., 0., 0., -0.05]))
        ...
        grasp_terminator_params.observation_mask = np.concatenate([
            ...
            manipulator_utils.FETCH_INDEX_DICT['gripper_state'],
            manipulator_utils.FETCH_INDEX_DICT['object_rot'],
            manipulator_utils.FETCH_INDEX_DICT['gripper_velp'],
            manipulator_utils.FETCH_INDEX_DICT['gripper_vel']
        ])
        ...
        def place_default_action(observations: th.Tensor):
            return th.where(observations[..., manipulator_utils.FETCH_INDEX_DICT['grasped']].unsqueeze(dim=-1) == 1.0,
                            th.Tensor([0., 0., 0., -0.05]),
                            th.Tensor([0., 0., 0., 0.])).squeeze()
        ...
        place_actor_params.action_mask = np.arange(3)
